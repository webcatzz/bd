...
func _tool_fill(event: InputEventMouse) -> void:
	if event is InputEventMouseButton and mouse_held:
		var tile: Tile = grid.get_tile(mouse_rect.position)
		if tile:
			stage_fill(tile.coords, tile.source_id, tile.tile_id)
			commit_paint()
	...
	if event is InputEventMouseButton and mouse_held:
		...
		if target:
			for tile: Tile in grid.tiles:
				if tile.source_id == target.source_id and tile.tile_id == target.tile_id:
					stage(tile.coords)
			commit_paint()
...
func stage(coords: Vector2i) -> void:
	if not is_staged(coords):
		staged_coords.insert(staged_coords.bsearch(coords), coords)
...
func stage_fill(coords: Vector2i, source_id: int, tile_id: Vector2i) -> void:
	stage(coords)
	for unit: Vector2i in [Vector2i.UP, Vector2i.DOWN, Vector2i.LEFT, Vector2i.RIGHT]:
		...
		if tile and tile.source_id == source_id and tile.tile_id == tile_id and not is_staged(tile.coords):
			stage_fill(tile.coords, source_id, tile_id)


func is_staged(coords: Vector2i) -> bool:
